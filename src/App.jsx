...
export default function AttendingPosterGenerator() {
  ...
  return (
    <div className="min-h-screen p-3 sm:p-4" style={{background: 'linear-gradient(to bottom right, #020617, #1E1B4B, #0F172A)'}}>
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-4 sm:mb-6">
          <h1 className="text-2xl sm:text-3xl font-bold text-white mb-1 sm:mb-2 tracking-wide">I Will Be Attending</h1>
          <p className="text-base sm:text-lg text-cyan-400 font-mono tracking-wider">&lt;PosterGenerator /&gt;</p>
        </div>

        <div className="grid lg:grid-cols-2 gap-4 sm:gap-8 items-start">
          {/* Editor Panel */}
          <div className="bg-slate-900/40 backdrop-blur-xl rounded-2xl sm:rounded-3xl p-4 sm:p-8 border border-white/5 shadow-2xl">
            <h2 className="text-xl sm:text-2xl font-bold text-white mb-3 sm:mb-4 tracking-tight">Your Details</h2>

            <div className="space-y-3 sm:space-y-4">
              <div>
                <label className="block text-slate-200 text-sm sm:text-base mb-1.5 sm:mb-2 font-medium">Upload Your Photo</label>
                <div className="relative">
                  <input
                    type="file"
                    accept="image/*"
                    onChange={handleImageUpload}
                    className="w-full p-2.5 sm:p-4 rounded-lg sm:rounded-xl bg-white/5 text-slate-300 text-sm sm:text-base border-2 border-cyan-500/20 hover:border-cyan-400/50 transition file:mr-3 sm:file:mr-4 file:py-1.5 sm:file:py-2 file:px-3 sm:file:px-4 file:rounded-full file:border-0 file:bg-gradient-to-r file:from-cyan-500 file:to-indigo-500 file:text-white file:text-xs sm:file:text-sm file:cursor-pointer file:font-semibold"
                  />
                </div>
                <p className="text-slate-400 text-xs sm:text-sm mt-1.5 sm:mt-2">Upload a clear, professional photo</p>
              </div>

              <div>
                <label className="block text-slate-200 text-sm sm:text-base mb-1.5 sm:mb-2 font-medium">Your Full Name</label>
                <input
                  type="text"
                  value={attendeeData.name}
                  onChange={(e) => setAttendeeData({ ...attendeeData, name: e.target.value })}
                  className="w-full p-2.5 sm:p-3 rounded-lg bg-white/5 text-white text-sm sm:text-base border-2 border-cyan-500/20 focus:border-cyan-500 outline-none transition"
                  placeholder="e.g., Eminent Augusta Ihezuo"
                />
              </div>

              <div>
                <label className="block text-slate-200 text-sm sm:text-base mb-1.5 sm:mb-2 font-medium">Your Title/Position</label>
                <input
                  type="text"
                  value={attendeeData.title}
                  onChange={(e) => setAttendeeData({ ...attendeeData, title: e.target.value })}
              ...
              <div>
                <label className="block text-slate-200 text-sm sm:text-base mb-1.5 sm:mb-2 font-medium">Select Design Style</label>
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-3">
                  {/* <button
                    onClick={() => setSelectedDesign(1)}
                    className={`p-2.5 sm:p-4 rounded-lg font-bold text-xs sm:text-base transition transform hover:scale-105 ${
                      selectedDesign === 1
                        ? 'text-white shadow-2xl'
                        : 'bg-white/10 text-white border-2 border-white/20 hover:border-cyan-500'
                    }`}
                    style={selectedDesign === 1 ? {background: 'linear-gradient(to bottom right, #06B6D4, #4F46E5)'} : {}}
                   > */}
                    {/* Design 1<br/>
                     <span className="text-[10px] sm:text-sm font-normal">Binary Pattern</span>
                   </button>
                   <button
                    ...
                    className={`p-2.5 sm:p-4 rounded-lg font-bold text-xs sm:text-base transition transform hover:scale-105 ${
                     ...
                     style={selectedDesign === 2 ? {background: 'linear-gradient(to bottom right, #06B6D4, #4F46E5)'} : {}}
                   > */}
                     {/* Design 2<br/>
                     <span className="text-[10px] sm:text-sm font-normal">Professional</span>
                   </button> */}
                  <button
                    ...
                    className={`p-2.5 sm:p-4 rounded-lg font-bold text-xs sm:text-base transition transform hover:scale-105 ${
                      selectedDesign === 3
                         ...
                         : 'bg-white/5 text-slate-300 border-2 border-white/10 hover:border-cyan-500/50'
                    }`}
                    style={selectedDesign === 3 ? {background: 'linear-gradient(to bottom right, #06B6D4, #4F46E5)'} : {}}
                  >
                    Poster Design<br/>
                    <span className="text-[10px] sm:text-sm font-normal opacity-90">Elegant Tech</span>
                  </button>
                  {/* <button
                    onClick={() => setSelectedDesign(4)}
                    className={`p-2.5 sm:p-4 rounded-lg font-bold text-xs sm:text-base transition transform hover:scale-105 ${
                    ...
                    style={selectedDesign === 4 ? {background: 'linear-gradient(to bottom right, #06B6D4, #4F46E5)'} : {}}
                  >
                    Design 4<br/>
                    <span className="text-[10px] sm:text-sm font-normal">Invitation</span>
                  </button> */}
              ...
              <button
                onClick={downloadPoster}
                className="w-full text-white py-2.5 sm:py-3 rounded-lg font-bold text-sm sm:text-base flex items-center justify-center gap-2 hover:shadow-cyan-500/20 hover:shadow-xl transition transform hover:scale-[1.02]"
                style={{background: 'linear-gradient(to right, #06B6D4, #4F46E5, #06B6D4)', backgroundSize: '200% auto'}}
              >
                <Download size={18} className="sm:w-5 sm:h-5" />
                Download Your Poster
              </button>
          ...
          {/* Preview Panel */}
          <div className="bg-slate-900/40 backdrop-blur-xl rounded-2xl sm:rounded-3xl p-4 sm:p-8 border border-white/5 shadow-2xl">
            <h2 className="text-xl sm:text-2xl font-bold text-white mb-3 sm:mb-4 tracking-tight">Live Preview</h2>
            ...
            <div className="mt-4 sm:mt-6 bg-cyan-500/5 border border-cyan-500/20 rounded-lg sm:rounded-xl p-3 sm:p-4">
              <p className="text-cyan-300/90 text-xs sm:text-sm text-center font-medium font-mono">
                // Click "Download Your Poster" to compile into a high-quality PNG
              </p>
            </div>
          </div>
        </div>
        ...
        {/* Instructions */}
        <div className="mt-4 sm:mt-6 bg-slate-900/40 backdrop-blur-xl rounded-xl sm:rounded-2xl p-4 sm:p-6 border border-white/5 shadow-2xl">
          <h3 className="text-base sm:text-lg font-bold text-white mb-2 sm:mb-3 tracking-tight"> // Quick Guide</h3>
          <div className="grid md:grid-cols-2 gap-3 sm:gap-4 text-slate-300">
            <div>
              <h4 className="font-bold text-cyan-400 mb-1.5 sm:mb-2 text-xs sm:text-sm font-mono">How to Create:</h4>
              <ol className="space-y-0.5 sm:space-y-1 list-decimal list-inside text-[11px] sm:text-xs opacity-90">
                <li>Upload a clear, professional headshot photo</li>
                <li>Enter your full name</li>
                <li>Add your job title or position</li>
                <li>Choose your preferred design style</li>
            ...
            <div>
              <h4 className="font-bold text-cyan-400 mb-1.5 sm:mb-2 text-xs sm:text-sm font-mono">Tips for Best Results:</h4>
              <ul className="space-y-0.5 sm:space-y-1 list-disc list-inside text-[11px] sm:text-xs opacity-90">
                <li>Use a high-quality photo with good lighting</li>
                <li>Plain or professional background works best</li>
                <li>Face should be clearly visible</li>
                <li>Keep title concise but descriptive</li>
